...
__global__ void upfirdn2d_kernel(scalar_t *out, const scalar_t *input, const scalar_t *kernel, const UpFirDn2DKernelParams p) {
	...
	constexpr int tile_in_h = ((tile_out_h - 1) * down_y + kernel_h - 1) / up_y + 1;
	constexpr int tile_in_w = ((tile_out_w - 1) * down_x + kernel_w - 1) / up_x + 1;

	// Shared kernel and shared x
	__shared__ volatile float sk[kernel_h][kernel_w];
	__shared__ volatile float sx[tile_in_h][tile_in_w];

	// Block Information (no checks on block_out_x < p.out_w cuz it will be checked on host)
	const int tile_out_y	  = blockIdx.x * tile_out_h;
	const int tile_out_x	  = blockIdx.y * tile_out_w;
	const int tile_out_n_base = blockIdx.z * p.loop_n;

	if (tile_out_x >= p.out_w | tile_out_y >= p.out_h | tile_out_n_base >= p.n) {
		return;
	}
...
torch::Tensor upfirdn2d_op(const torch::Tensor &input,
	...
	auto out =
		at::empty({p.n, 1, p.out_h, p.out_w}, x.options());
