...
random_img = torch.rand((4096, 1, 64, 64), dtype=torch.float32, device="cuda")
...
def fn(i: int) -> None:
    ...
    new_result = new(random_img, k, up=1, down=1, pad=(2, 2), force_generic=False)
    new_result_generic = new(
        random_img, k, up=1, down=1, pad=(2, 2), force_generic=True
    )
